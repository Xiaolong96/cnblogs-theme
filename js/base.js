...
function Base() {
    ...
    this.init = function () {
        ...
        // Loading前初始化
        xlupcJs.loadingBeforeInit();

        // Loading结束
        xlupcJs.endLoading();

        // Loading后初始化
        xlupcJs.loadingAfterInit();
    };
    ...
    this.loadingBeforeInit = function () {
        ...
        // 页面初始化
        ($('#topics').length > 0) ? xlupcJs.notHomeInit() : xlupcJs.homeInit();
    };
    ...
    this.loadingAfterInit = function () {
        ...
        // 滚动监听
        $(window).scroll( function() { xlupcJs.scrollMonitor(); });

        // 窗口变化监听
        $(window).resize( function() { xlupcJs.resizeMonitor(); });
        ...
        // 设置菜单侧边栏内容
        timeIds.setMenuDataTId = window.setInterval( xlupcJs.setMenuData, 1000 );

        // html5-title
        xlupcJs.htmlTitle();

        // 添加页面特效控制
        xlupcJs.setPageAnimationControl();
    ...
    this.homeInit = function() {
        ...
        // 设置右下角菜单
        timeIds.setHomeRightMenuTId = window.setInterval( xlupcJs.addHomeRightMenu, 1000 );

        xlupcJs.setHitokoto();
        xlupcJs.scrollMonitor();
        xlupcJs.setDomHomePosition();
    ...
    this.notHomeInit = function() {
        ...
        require(['baguetteBox', 'marvin', 'articleStatement'], function(baguetteBox) {
            ...
            // 初始化文章目录位置
            timeIds.setCatalogTId = window.setInterval( xlupcJs.initCatalog, 1000 );

            xlupcJs.scrollMonitor();
        });

        // 设置右下角菜单
        timeIds.setNotHomeRightMenuTId = window.setInterval( xlupcJs.addNotHomeRightMenu, 1000 );

        xlupcJs.setNotHomeTopImg();
        xlupcJs.setCommentStyle();
    };
    ...
    this.initCatalog = function() {
        ...
        if (sideToolbar.length > 0) {
            ...
            xlupcJs.resizeMonitor();
            sideToolbar.fadeIn(300);
            xlupcJs.clearIntervalTimeId(timeIds.setCatalogTId);
        }
    };
        ...
        function setScrollbarStyle() {
            ...
            var scrollbarTimeId = window.setInterval( function () {
                if ($('.post pre span').length > 0) {
                    ...
                    xlupcJs.clearIntervalTimeId(scrollbarTimeId);
                }
            }, 500 );
        }
    };
    ...
    this.addFooter = function() {
        ...
        pvHtml += '<div>I love waking up in the morning and not knowing what\'s going to happen， or who I\'m going to meet， where I\'m going to wind up.</div>'
        ...
        //         xlupcJs.clearIntervalTimeId(timeIds.setAmazingTId);
        //     }
        // }
    };
    ...
    this.setMenuData = function() {
        ...
        if (
            ...
            xlupcJs.clearIntervalTimeId(timeIds.setMenuDataTId);
        }
    ...
    this.addHomeRightMenu = function() {
        ...
        if (rightMenu.length > 0) {
            ...
            xlupcJs.rightMenuMous('#attention', '.attentionSpan');
            xlupcJs.scrollMonitor(); // 触发一次滚动处理，防止未有对象，初始化失败
            xlupcJs.clearIntervalTimeId(timeIds.setHomeRightMenuTId);
        }
    };
